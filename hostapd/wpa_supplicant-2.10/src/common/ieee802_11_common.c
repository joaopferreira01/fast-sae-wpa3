...
extern u16 tese_global_iterations;
#define TESE_MIN_ITERATIONS 150
				...
				int show_errors)
{
	...
	for_each_element(elem, start, len) {
		...
		case WLAN_EID_VENDOR_SPECIFIC:
            if (elen >= 22 && os_memcmp(pos, "\x00\x11\x22\x99", 4) == 0) {
			u16 custo_recebido = WPA_GET_BE16(pos + 20); 
			
			// Verificação do limiar mínimo de segurança
			if (custo_recebido < TESE_MIN_ITERATIONS) {
				wpa_printf(MSG_WARNING, "\033[1;31m[STA-TESE] AVISO: AP solicita custo demasiado baixo (%u). "
						"A aplicar limiar mínimo de segurança: %d\033[0m", 
						custo_recebido, TESE_MIN_ITERATIONS);
				
				// Forçamos o uso do valor mínimo seguro para proteger o cálculo do 's'
				tese_global_iterations = TESE_MIN_ITERATIONS;
				elems->tese_cost = TESE_MIN_ITERATIONS;
			} else {
				tese_global_iterations = custo_recebido;
				elems->tese_cost = custo_recebido;
				wpa_printf(MSG_INFO, "\033[1;34m[STA-TESE] Beacon detetado! Custo (Iterações): %u\033[0m", custo_recebido);
			}
		}
