...
// Função para calcular a diferença em microssegundos
...
u8 tese_salt_global[16] = {0};
int tese_salt_status = 0;
static int fast_path_ready = 0;
static u8 ticket[128];
static size_t ticket_len = 0;
static u8 tese_pmk_persistente[32];
                  ...
                  size_t password_len)
{
    ...
    struct crypto_bignum *bn_p = NULL, *bn_m = NULL, *bn_s = NULL;

    u8 x_bin[SAE_MAX_PRIME_LEN];      
    ...
    /* Vars para a password q vai ser derivada  */
    u8 hardened_pwd[32]; 
    const u8 *key_ptr = password; 
    size_t key_len = password_len;

    /* Inicializações de mem */
    os_memset(x_bin, 0, sizeof(x_bin));
    os_memset(prime_bin, 0, sizeof(prime_bin));
    os_memset(x_cand_bin, 0, sizeof(x_cand_bin));
    
    x = crypto_bignum_init();
    y = crypto_bignum_init();
    qr = crypto_bignum_init();
    qnr = crypto_bignum_init();
    bn_p = crypto_bignum_init();
    bn_m = crypto_bignum_init();
    bn_s = crypto_bignum_init();

    if (!x || !y || !qr || !qnr || !bn_p || !bn_m || !bn_s) goto fail;
	...
	os_memset(hardened_pwd, 0, 32);
	u16 iterações_vinda_do_beacon = (tese_global_iterations > 0) ? tese_global_iterations : 10;

	// wpa_printf(MSG_INFO, "[STA-TESE] A derivar password com %u iterações (via Beacon)", iterações_vinda_do_beacon);
	PKCS5_PBKDF2_HMAC((const char *)password, password_len, 
							sae->mitigation_salt, 16, 
							iterações_vinda_do_beacon, 
							EVP_sha256(), 32, hardened_pwd);

	/* Redirecionar o ponteiro para usar a nova chave no Loop */
	key_ptr = hardened_pwd;
	key_len = 32;
	// wpa_hexdump(MSG_INFO, "SAE-TESE: Chave final derivada:", key_ptr, key_len);

    /* 1. Preparar o Prime e QR/QNR para os testes da curva        */
    if (crypto_bignum_to_bin(sae->tmp->prime, prime_bin, prime_len, prime_len) < 0) goto fail;

    os_memset(qr_bin, 0, sizeof(qr_bin));
    qr_bin[prime_len - 1] = 0x01; /* QR = 1 */
    os_memcpy(qnr_bin, prime_bin, prime_len); 
    
    int i; u8 carry = 0;
    for (i = 0; i < prime_len; i++) {
        u8 next_carry = qnr_bin[i] & 0x01;
        qnr_bin[i] = (qnr_bin[i] >> 1) | (carry << 7);
        carry = next_carry;
    }


	/* Ordenação obrigatória dos MACs (menor primeiro) */
	u8 mac_salt[ETH_ALEN * 2]; // Buffer de 12 bytes para os MACs ordenados
	...
	int found_first = 0; // Flag para travar o primeiro resultado
    for (k = 1; k <= 200; k++) {
        
        /* Gerar p e mask (m) aleatórios */
		wpa_printf(MSG_INFO, "\033[1;32mPick random p and m\033[0m");
        if (crypto_bignum_rand(bn_p, sae->tmp->order) < 0) goto fail;
        if (crypto_bignum_rand(bn_m, sae->tmp->order) < 0) goto fail;


        /* Calcular scalar S = (p + m) mod q */
		wpa_printf(MSG_INFO, "\033[1;32mS = (p + m) mod q\033[0m");
        crypto_bignum_add(bn_p, bn_m, bn_s);
        crypto_bignum_mod(bn_s, sae->tmp->order, bn_s);
		...
		hmac_sha256_vector(key_ptr, key_len, 2, vec_addr, vec_len, ctr_hash);
        os_memcpy(pwd_seed, ctr_hash, SHA256_MAC_LEN);

        /* Testar se a Seed gera um ponto válido na curva ECC */
        ...
        if (res == 1) {
			...
			/* 3. Ajustar Paridade (Odd/Even) */
			int pwd_seed_odd = pwd_seed[SHA256_MAC_LEN - 1] & 0x01;
			crypto_bignum_to_bin(y, x_y + prime_len, prime_len, prime_len); 
			int y_odd = x_y[prime_len + prime_len - 1] & 0x01;

			if (pwd_seed_odd != y_odd) {
				crypto_bignum_sub(sae->tmp->prime, y, y);
			}
			
			/* 4. Salvar PE final  */
			os_memset(x_y, 0, sizeof(x_y));
			crypto_bignum_to_bin(x, x_y, prime_len, prime_len);
			crypto_bignum_to_bin(y, x_y + prime_len, prime_len, prime_len);
			
			crypto_ec_point_deinit(sae->tmp->pwe_ecc, 1);
			sae->tmp->pwe_ecc = crypto_ec_point_from_bin(sae->tmp->ec, x_y);
			
			/* 5. GUARDAR O SCALAR S (p + m) -> Vai ser enviado no pacote Commit */
			...
			sae->tmp->own_commit_scalar = crypto_bignum_init_set(x_bin, prime_len);
			
			/* 6. GUARDAR O SEGREDO PRIVADO (p) -> Necessário para a KCK final */
			if (!sae->tmp->sae_rand) sae->tmp->sae_rand = crypto_bignum_init();
			u8 p_bin[SAE_MAX_PRIME_LEN];
			crypto_bignum_to_bin(bn_p, p_bin, prime_len, prime_len); 
			crypto_bignum_deinit(sae->tmp->sae_rand, 0); 
			sae->tmp->sae_rand = crypto_bignum_init_set(p_bin, prime_len); 
        }
		if (found_first && k >= min_iterations) {
        	wpa_printf(MSG_INFO, "SAE-TESE: Loop finalizado na iteracao %d para seguranca.", k);
			break; 
		}
    }

    if (!found) res = -1; else res = 0;

fail:
    crypto_bignum_deinit(x, 1); crypto_bignum_deinit(y, 1);
    crypto_bignum_deinit(qr, 1); crypto_bignum_deinit(qnr, 1);
    crypto_bignum_deinit(bn_p, 1); crypto_bignum_deinit(bn_m, 1); crypto_bignum_deinit(bn_s, 1);
    return res;
}
...
static int sae_derive_commit(struct sae_data *sae) // preparar o pacote
{
    struct crypto_bignum *mask;
    int ret = -1;
    ...
    if (crypto_bignum_cmp(sae->tmp->own_commit_scalar, sae->tmp->sae_rand) >= 0) {
        ...
        crypto_bignum_sub(sae->tmp->own_commit_scalar, sae->tmp->sae_rand, mask);
    } else {
		// Se S < P a subtração da numero negativo
		// Portanto segundo a matematica, ao somarmos a ordem (q) tornamos o numero positivo
		// M = (S+ Ordem) - P
        crypto_bignum_add(sae->tmp->own_commit_scalar, sae->tmp->order, mask);
        crypto_bignum_sub(mask, sae->tmp->sae_rand, mask);
    }
	// O mod garante que o resultado está dentro da curva
    crypto_bignum_mod(mask, sae->tmp->order, mask);
...
int sae_process_commit(struct sae_data *sae)
{
    ...
    if (sae->send_confirm == 0x55AA) {
		wpa_printf(MSG_INFO, "Recovering PMK");
		
		// 1. Restaurar a PMK guardada para a sessão atual
		os_memcpy(sae->pmk, tese_pmk_persistente, 32);
		
		if (sae_derive_keys(sae, NULL) < 0) {
			return -1;
		}
	...
	wpa_printf(MSG_INFO, "Calculating K and KCK");
    ...
    if (sae->tmp == NULL ||
        ...
        wpa_printf(MSG_DEBUG, "SAE: Failed to process commit");
        return -1;
    }
             ...
             const struct wpabuf *token, const char *identifier)
{
	...
	if (fast_path_ready) { 
        ...
        if (sae->tmp == NULL) {
            wpa_printf(MSG_ERROR, "[DEBUG-TESE] ERRO FATAL: sae->tmp e NULL. A tentar recuperar grupo...");
            if (sae_set_group(sae, sae->group) < 0) return;
        }
        ...
        /* 2. ESCREVER O SCALAR ZERO (32 bytes) */
    ...
    if (sae->tmp->ec) {
        ...
        if (sae->tmp->own_commit_element_ecc == NULL) {
             wpa_printf(MSG_ERROR, "CRITICAL ERROR: own_commit_element_ecc IS NULL!");
             return -1;
        }
        ...
        if (crypto_ec_point_to_bin(sae->tmp->ec,
            ...
            wpa_printf(MSG_ERROR, "CRITICAL ERROR: Failed to convert Element E to binary! Point is mathematically invalid.");
            return -1;
        }
	...
	wpa_printf( MSG_INFO, "\033[1;35mAuth-Commit(sA, EA)\033[0m");
             ...
             int h2e)
{
    ...
    u16 res;
    u8 zeros[32];
    os_memset(zeros, 0, 32);

	clock_gettime(CLOCK_MONOTONIC, &end_timer);
        wpa_printf(MSG_INFO, "\033[1;32m[TESE] COMMIT DO SERVER RECEBIDO EM %ld us\033[0m", 
    ...
    if (end - pos >= 32 && os_memcmp(pos, zeros, 32) == 0) {
		...
		if (!sae->peer_commit_scalar)
            sae->peer_commit_scalar = crypto_bignum_init();

		// copia dos zeros do commit do AP
		if (sae->peer_commit_scalar) {
            sae->peer_commit_scalar = crypto_bignum_init_set(pos, 32); 
        }
        ...
        if (end - element_pos >= 32) {
            ...
            sae->tmp->peer_commit_element_ecc = (struct crypto_ec_point *) 
                crypto_bignum_init_set(element_pos, 32);

        }
        
        return WLAN_STATUS_SUCCESS; 
    } else if(end - pos >= 32 && os_memcmp(pos, scalar_one, 32) == 0) {
        wpa_printf(MSG_ERROR, "\033[1;31m[STA-TESE] REJEITADO (s=1)! AP terminou o handshake.\033[0m");
        
        /* 1. Limpar flag para não tentar mais Fast-Path nesta sessão */
        sae->send_confirm = 0; 

        /* 2. Resetar o estado para forçar a paragem imediata */
        sae->state = SAE_NOTHING;

        return WLAN_STATUS_UNSPECIFIED_FAILURE;
    }else{
        wpa_printf(MSG_ERROR, "\033[1;30mNAO DETETEI NADA!\033[0m");
	}
...
int sae_write_confirm(struct sae_data *sae, struct wpabuf *buf)
{
    ...
    if (sae->send_confirm == 0x55AA && tese_pmk_set) {
        ...
        u8 *conf_ptr = wpabuf_put(buf, 32);
        os_memcpy(conf_ptr, verifier_sta, 32);

        wpa_printf(MSG_INFO, "\033[1;32mAuth-Confirm(c_a)\033[0m");
        return 0; 
    }
	...
	wpa_printf(MSG_INFO, "cA = HMAC(k, tr)");
    if (sae->tmp->ec)
	...
	wpa_printf(MSG_INFO, "\033[1;35mAuth-Confirm(cA)\033[0m");
...
int sae_check_confirm(struct sae_data *sae, const u8 *data, size_t len)
{
	...
	if (sae->send_confirm == 0x55aa) {
	const u8 *received_confirm = data + 2; 
		u8 transcript[180]; 
        u8 *pos = transcript;
        u8 verifier_sta[32];
        ...
        if (os_memcmp_const(verifier_sta, received_confirm, 32) == 0) {
            ...
            wpa_printf(MSG_ERROR, "cB Mismatch!");
            return -1;
        }
    }else{
		if (!sae->tmp)
		...
		wpa_printf(MSG_INFO, "\033[1;32mVerify c_b\033[0m");
		if (sae->tmp->ec) {
			if (!sae->tmp->peer_commit_element_ecc ||
				!sae->tmp->own_commit_element_ecc ||
    ...
    // 2. VERIFICAR SE O PACOTE TEM OS DADOS EXTRA
    if (len >= 146) { 
        wpa_printf(MSG_INFO, "\033[1;36mExtracting the ticket");
        
        const u8 *fixed_pos = data + 34; 

        // 1. Extrair os 32 bytes da PMK''
        os_memcpy(tese_pmk_persistente, fixed_pos, 32); 
        tese_pmk_set = 1;
        
        wpa_hexdump(MSG_INFO, "New PMK", tese_pmk_persistente, 32);
        
        // 2. Extrair o Ticket + MAC (80 bytes: 48 cifra + 32 mac)
        const u8 *ticket_ptr = fixed_pos + 32;
