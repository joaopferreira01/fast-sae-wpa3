...
#ifdef CONFIG_SAE
/* Função para encontrar o nosso IE de mitigação (Salt) nos IEs do Beacon */
static const u8 * find_mitigation_ie(const u8 *ies, size_t len)
{
	const u8 *pos = ies;
	...
	while (pos + 2 <= end) {
		...
		/* 221 (0xdd) é o ID para Vendor Specific IEs */
		if (ie_id == 221 && ie_len >= 16) {
			/* Aqui deves verificar se o OUI/dados batem com o que puseste no hostapd */
			/* Se no hostapd usaste o OUI 00:11:22, verificas: */
			if (os_memcmp(&pos[2], "\x00\x11\x22", 3) == 0) {
				return pos;
			}
		}
		pos += 2 + ie_len;
              ...
              struct wpa_bss *bss, struct wpa_ssid *ssid)
{
    ...
    const u8 *mitigation_ie;
    
    /* bss + 1 aponta para o início dos IEs guardados do Beacon/Probe Scan */
    mitigation_ie = find_mitigation_ie((const u8 *) (bss + 1), bss->ie_len);

    if (mitigation_ie && bss->ie_len >= 18) { // 2 cabeçalho + 16 bytes do salt
		/* +6 para saltar ID(1), LEN(1), OUI(3) e o byte extra de desalinhamento */
		os_memcpy(tese_salt_global, mitigation_ie + 6, 16);
		tese_salt_status = 1;
		wpa_hexdump(MSG_INFO, "Valor do salt vindo do AP:", tese_salt_global, 16);
	} else {
        wpa_printf(MSG_INFO, "SME-TESE: Não encontrei o IE com o salt");
        wpa_s->sme.sae.mitigation_enabled = 0;
    }
#endif /* CONFIG_SAE */
