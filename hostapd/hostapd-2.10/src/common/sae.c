...
long get_diff_micro(struct timespec start, struct timespec end) {
...
void update_tid_window(u32 received_tid) {
                       ...
                       u8 *out, size_t *out_len)
{
    ...
    // Ciframos os dois blocos de 16 bytes (total 32)
    aes_encrypt(ctx, (u8 *)payload, out);
    aes_encrypt(ctx, (u8 *)payload + 16, out + 16);

	aes_encrypt(ctx, (u8 *)payload + 32, out + 32);
...
int sae_decrypt_ticket(const u8 *stek, const u8 *cipher, size_t cipher_len, 
                       struct sae_ticket_payload *payload)
{
    u8 computed_mac[32];
    u8 plain[48]; 
    ...
    size_t encrypted_len = 48; 
    const u8 *received_mac = cipher + encrypted_len;
    ...
    /* 2. Decifrar os 3 blocos AES (48 bytes exatos) */
    void *ctx = aes_decrypt_init(stek, 16);
    if (!ctx) return -1;
    ...
    // O TID ocupa os primeiros 4 bytes [0,1,2,3]
    os_memcpy(&payload->tid, plain, 4);
    
    // A PMK ocupa os 32 bytes seguintes [4 até 35]
    os_memcpy(payload->pmk_prime, plain + 4, 32); 

    wpa_printf(MSG_INFO, "\033[1;32mTicket Validated");
    
    // wpa_hexdump(MSG_INFO, "[AP-DEBUG] PMK Recuperada (32 bytes):", payload->pmk_prime, 32);
...
// Espera pelos valores vindos do commit. Aqui seria o hunt n peck, mas o servidor n precisa de fazer
               ...
               struct sae_data *sae)
{
    ...
    u8 ctr_hash[32];
    u8 pwd_seed[32];
    u8 scalar_bin_tmp[SAE_MAX_PRIME_LEN];
    ...
    struct crypto_bignum *x_tmp = NULL, *y_tmp = NULL, *mask = NULL;
    size_t prime_len;
    int i;
    u8 carry = 0;
    ...
    /* 1. Guardar os macs */
    if (sae->tmp) {
        os_memcpy(sae->tmp->own_addr, addr1, ETH_ALEN);
        os_memcpy(sae->tmp->peer_addr, addr2, ETH_ALEN);
    }

    /* 2. Inicializar bignums */
    if (!sae->tmp->sae_rand) sae->tmp->sae_rand = crypto_bignum_init();
    if (!sae->tmp->own_commit_scalar) sae->tmp->own_commit_scalar = crypto_bignum_init();

    /* 3. VERIFICAÇÃO: Se já temos o scalar do cliente, validamos o PE */
    if (sae->peer_commit_scalar && !crypto_bignum_is_zero(sae->peer_commit_scalar)) {
		...
		if (sae->tese_cache_ready) {
			...
			PKCS5_PBKDF2_HMAC((const char *)password, password_len, 
							tese_salt_global, 16, 
							cost, 
							EVP_sha256(), 32, sae->tese_hardened_pwd_cache);
			
			os_memcpy(hardened_pwd, sae->tese_hardened_pwd_cache, 32);
			sae->tese_cache_ready = 1;
		}

		wpa_printf(MSG_INFO, "Validating PE");
        clock_gettime(CLOCK_MONOTONIC, &start_timer);
		// cada curva (P-256 do grupo 19) é definida por um numero primo muito grade
		prime_len = sae->tmp->prime_len; // Tamanho do número primo do grupo (32 bytes para o Grupo 19)
		...
		// grupo 19 o primo tem 256 bits, logo 32 bytes, q é o tamanho do s e das coordenadas
        const u8 *vec_addr[2];
        size_t vec_len[2];
        vec_addr[0] = scalar_bin_tmp;  	vec_len[0] = prime_len;
        vec_addr[1] = addrs;        	vec_len[1] = 12;

		// seed
		wpa_printf(MSG_INFO, "pwd_seed = HMAC(hard_pwd, s_a, MACs)");
        hmac_sha256_vector(hardened_pwd, 32, 2, vec_addr, vec_len, ctr_hash);
        os_memcpy(pwd_seed, ctr_hash, 32);


        // verificar se a seed corresponde a um ponto valido x na curva
        if (sae_test_pwd_seed_ecc(sae, pwd_seed, prime_bin, qr_bin, qnr_bin, x_cand_bin) != 1) {
            wpa_printf(MSG_ERROR, C_RED "TESE: Falha na validação da seed. Não gera ponto válido!" C_RESET);
            return -1;
        }

        x_tmp = crypto_bignum_init_set(x_cand_bin, prime_len); //
        y_tmp = crypto_ec_point_compute_y_sqr(sae->tmp->ec, x_tmp);
        if (!y_tmp || dragonfly_sqrt(sae->tmp->ec, y_tmp, y_tmp) < 0) {
            return -1;
        }

        int pwd_seed_odd = pwd_seed[31] & 0x01;
        u8 y_bin[SAE_MAX_PRIME_LEN];
        crypto_bignum_to_bin(y_tmp, y_bin, prime_len, prime_len);
        if (pwd_seed_odd != (y_bin[prime_len - 1] & 0x01))
            crypto_bignum_sub(sae->tmp->prime, y_tmp, y_tmp);

        os_memset(x_y, 0, sizeof(x_y));
        crypto_bignum_to_bin(x_tmp, x_y, prime_len, prime_len);
        crypto_bignum_to_bin(y_tmp, x_y + prime_len, prime_len, prime_len);
        ...
        wpa_printf(MSG_INFO, "\033[1;32m[TESE] PE Validado e Resposta Gerada em %ld us\033[0m", 
                   get_diff_micro(start_timer, end_timer));

        /* F. GERAR RESPOSTA DO AP (Dragonfly Normal) */
        mask = crypto_bignum_init();
		wpa_printf(MSG_INFO, "Choose random p and m");
        crypto_bignum_rand(sae->tmp->sae_rand, sae->tmp->order); 
        crypto_bignum_rand(mask, sae->tmp->order);               
        // s = (p + m) mod q
		wpa_printf(MSG_INFO, "s = (p + m) mod q");
		...
		wpa_printf(MSG_INFO, "E = -m * PE");
        sae_derive_commit_element_ecc(sae, mask);

		// limpar
        crypto_bignum_deinit(mask, 1);
        crypto_bignum_deinit(x_tmp, 1);
        crypto_bignum_deinit(y_tmp, 1);
...
static int sae_derive_keys(struct sae_data *sae, const u8 *k)
{
    ...
    if (sae->send_confirm == 0x55AA) {
		// wpa_printf(MSG_INFO, "\033[1;32m[TESE] 2. Fast-Path: A derivar KCK e PMK...\033[0m");
		
		u8 chaves_derivadas[64];

		// wpa_printf(MSG_INFO, "\033[1;36m[TESE] A verificar argumentos da PRF...\033[0m");
		wpa_hexdump(MSG_INFO, "PMK:", sae->pmk, 32);

		sha256_prf(sae->pmk, 32, "SAE KCK and PMK", NULL, 0, chaves_derivadas, 64);

		wpa_hexdump(MSG_INFO, "KCK:", chaves_derivadas, 32);

		// 1. KCK para o AP validar o Confirm do STA
		os_memcpy(sae->tmp->kck, chaves_derivadas, 32); 
		sae->tmp->kck_len = 32;

		// 2. PMK para o AP usar no 4-Way Handshake
		os_memcpy(sae->pmk, chaves_derivadas + 32, 32);

		// wpa_hexdump(MSG_INFO, "[TESE] KCK-AP-FINAL", sae->tmp->kck, 32);
		return 0; 
	}
...
#endif /* CONFIG_SAE_PK */
	sae->tmp->kck[0], sae->tmp->kck[1], sae->tmp->kck[2], sae->tmp->kck[3];

    if (sae->tmp->kck[0] == 0 && sae->tmp->kck[1] == 0) {
        wpa_printf(MSG_INFO, "SAE-ERRO: A KCK continua a zeros na estrutura!");
    } else {
    }
...
int sae_process_commit(struct sae_data *sae)
{
    ...
    /* --- SAE-TESE: FAST-PATH --- */
    if (sae->send_confirm == 0x55AA) {
        // wpa_printf(MSG_INFO, "\033[1;32m[TESE] Fast-Path: A saltar a derivacao de 'k'. A usar a PMK do ticket.\033[0m");
        
        // Chamamos diretamente a derivacao de chaves KCK/KEK
        // Passamos NULL no 'k' porque a sae_derive_keys  
        // vai ignorar esse parametro e usar a sae->pmk do ticket.
        return sae_derive_keys(sae, NULL);
    }

    wpa_printf(MSG_INFO, "\033[1;32mCalculating K and KCK\033[0m");
             ...
             const struct wpabuf *token, const char *identifier)
{
    ...
    /* --- FLUXO NORMAL: DRAGONFLY / SAE PADRÃO --- */
    ...
    if (sae->tmp->own_commit_scalar == NULL || 
    ...
    if (sae->tmp->ec) {
        ...
        pos = wpabuf_put(buf, sae->tmp->prime_len);
        if (sae->tmp->own_commit_element_ffc == NULL ||
	...
	wpa_printf(MSG_INFO, "\033[1;36mAuth-Commit (sB, EB)\033[0m");
    return 0;
}
             ...
             int h2e)
{
    ...
    if (sae->tmp == NULL) {
        sae_set_group(sae, WPA_GET_LE16(data)); 
    }
    
    /* --- 1. VERIFICAÇÃO DO TICKET --- */
    if (len >= 34) {
        u8 zero_block[32];
        os_memset(zero_block, 0, 32);

        /* Se o Scalar for Zero, é uma tentativa de reautenticação com o  Ticket */
        if (os_memcmp(data + 2, zero_block, 32) == 0) {
            clock_gettime(CLOCK_MONOTONIC, &start_timer);

            const u8 *ticket_real_ptr = data + 34; 
            size_t ticket_real_len = 80;
            struct sae_ticket_payload payload;

            if (sae_decrypt_ticket(global_stek_enc, ticket_real_ptr, ticket_real_len, &payload) == 0) {
                u32 incoming_tid = be_to_host32(*(u32 *)((u8 *)&payload)); 
				wpa_printf(MSG_INFO, "\033[1;31mTID Check\033[0m");
                if (verify_tid(incoming_tid) != 0) {
                    wpa_printf(MSG_ERROR, "\033[1;31m[TESE] REPLAY DETETADO! TID %u\033[0m", incoming_tid);
                    goto force_custom_error;
                }

                update_tid_window(incoming_tid);
				wpa_printf(MSG_INFO, "\033[1;31mPMK Recovery\033[0m");
                os_memset(sae->pmk, 0, 32);
				os_memcpy(sae->pmk, payload.pmk_prime, 32); 
                ...
                sae->tmp->peer_commit_element_ecc = (struct crypto_ec_point *) 
                    crypto_bignum_init_set(ticket_real_ptr, 80);
                
                sae->send_confirm = 0x55AA; 
                if (token_len) *token_len = 0;

                clock_gettime(CLOCK_MONOTONIC, &end_timer);
                wpa_printf(MSG_INFO, "\033[1;33mTIMER_DATA:REAUTH_TICKET:%ld\033[0m", 
                           get_diff_micro(start_timer, end_timer));
                
                return 0; 
            } else {
                wpa_printf(MSG_ERROR, "\033[1;31m[TESE] TICKET INVÁLIDO!\033[0m");
            }
        }
    }
...
force_custom_error:
    sae->send_confirm = 0; 
    if (sae->tmp == NULL) sae_set_group(sae, WPA_GET_LE16(data));
    if (sae->tmp->own_commit_scalar) crypto_bignum_deinit(sae->tmp->own_commit_scalar, 1);

    u8 scalar_one[32];
    os_memset(scalar_one, 0, 32);
    scalar_one[31] = 0x01; 
    sae->tmp->own_commit_scalar = crypto_bignum_init_set(scalar_one, 32);
    return WLAN_STATUS_UNSPECIFIED_FAILURE;
        
skip_fast_path:
    /* Extração dos IEs do Commit */
    if (end - pos < 2) return WLAN_STATUS_UNSPECIFIED_FAILURE;
    res = sae_group_allowed(sae, allowed_groups, WPA_GET_LE16(pos));
    if (res != WLAN_STATUS_SUCCESS) return res;
    pos += 2;

    sae_parse_commit_token(sae, &pos, end, token, token_len, h2e);
    
    /* sae_parse_commit_scalar guarda o valor em sae->peer_commit_scalar */
...
int sae_write_confirm(struct sae_data *sae, struct wpabuf *buf)
{
    ...
    if (sae->send_confirm == 0x55AA) {
		...
		if (sae->tmp && sae->tmp->peer_commit_element_ecc) {
			sae->tmp->peer_commit_element_ecc = NULL; 
		}
		if (sae->tmp && sae->tmp->own_commit_element_ecc) {
			sae->tmp->own_commit_element_ecc = NULL;
		}
        ...
        /* CASO NORMAL (DRAGONFLY) */
        if (sae->send_confirm < 0xffff)
            sae->send_confirm++;
        
        wpabuf_put_le16(buf, sae->send_confirm);
        const u8 *sc = wpabuf_head_u8(buf) + wpabuf_len(buf) - 2;

        conf_field = wpabuf_put(buf, hash_len);
	    wpa_printf(MSG_INFO, "tr = (sA, EA, sB, EB)");
	    wpa_printf(MSG_INFO, "cB = HMAC(k, tr)");
        if (sae->tmp->ec)
        ...
        if (res) {
            wpa_printf(MSG_ERROR, "TESE-ERRO: Falha ao gerar o Confirm!");
            return res;
        }
    }

    /* --- 2. RENOVAÇÃO DO TICKET --- */
    u8 pmk_prime_nova[32]; // Nome diferente para evitar conflitos
    struct sae_ticket_payload payload; 
    u8 ticket_cifrado[48]; 
    u8 ticket_mac[32];
    size_t t_len = 48; 

    os_get_random(pmk_prime_nova, 32);
    os_memset(&payload, 0, sizeof(payload));
    
    payload.tid = host_to_be32(global_tid_counter++);
    os_memcpy(payload.pmk_prime, pmk_prime_nova, 32);

	if (sae_encrypt_ticket(global_stek_enc, &payload, ticket_cifrado, &t_len) == 0) {
		hmac_sha256(global_stek_mac, 16, ticket_cifrado, 48, ticket_mac);
        wpabuf_put_data(buf, pmk_prime_nova, 32);     // 32 bytes
        wpabuf_put_data(buf, ticket_cifrado, 48);      // 48 bytes
...
int sae_check_confirm(struct sae_data *sae, const u8 *data, size_t len)
{
    ...
    if (sae->send_confirm == 0x55AA) {
        ...
        if (sae->peer_commit_scalar) {
            ...
            wpa_printf(MSG_ERROR, "s_a é NULL!");
            os_memset(pos, 0, 32);
        }
        pos += 32;
        ...
        if (sae->tmp && sae->tmp->ec && sae->tmp->own_commit_element_ecc) {
            ...
            wpa_printf(MSG_ERROR, "E_a é NULL!");
            os_memset(pos, 0, 32);
        }
        pos += 32;
        ...
        if (os_memcmp_const(verifier_ap, received_confirm, 32) == 0) {
            ...
            wpa_printf(MSG_INFO, "Generate new Ticket and PMK'");

            // Derivar a PMK'' (próxima chave) a partir da PMK atual
            u8 next_pmk[32];
            hmac_sha256(sae->pmk, 32, sae->pmk, 32, next_pmk);

            // Gerar um novo TID aleatório
            u32 next_tid;
            os_get_random((u8 *) &next_tid, sizeof(next_tid));

            // Salvar para o sae_write_confirm do AP
            sae->next_tid = next_tid;
            os_memcpy(sae->next_pmk, next_pmk, 32);
            sae->has_next_ticket = 1;


            return 0;
        } else {
            wpa_printf(MSG_ERROR, "\033[1;31m[AP-TESE] Confirm Mismatch! Intruso ou erro de derivação.\033[0m");
            return -1;
        }
    }

    /* FLUXO NORMAL (DRAGONFLY) */
    hash_len = sae->tmp->kck_len;
    if (len < 2 + hash_len) {
	...
	wpa_printf(MSG_INFO, "\033[1;32mVerify cA\033[0m");
    if (sae->tmp->ec) {
        if (!sae->tmp->peer_commit_element_ecc ||
            !sae->tmp->own_commit_element_ecc ||
