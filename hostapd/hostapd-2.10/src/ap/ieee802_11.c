					      ...
					      struct sta_info *sta)
{
	...
	buf = wpabuf_alloc(SAE_CONFIRM_MAX_LEN + 500);
	if (buf == NULL)
		return NULL;
			    ...
			    u16 auth_transaction, u16 status_code)
{
	...
	if (auth_transaction == 1) {
        ...
        if (sta->sae->state >= SAE_CONFIRMED ||
            ...
            // Verificação do Confirm
            if (sae_check_confirm(sta->sae, var, var_len) < 0) {
                resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
                goto reply;
            }

            /* --- TESE: Sincronização Fast-Path --- */
            if (sta->sae->send_confirm == 0x55AA) {
                // 1. Forçamos o estado para Aceite
        ...
        // Deixamos o motor original correr para ele enviar o Passo 4 (Confirm do AP)
        resp = sae_sm_step(hapd, sta, mgmt->bssid, auth_transaction,
