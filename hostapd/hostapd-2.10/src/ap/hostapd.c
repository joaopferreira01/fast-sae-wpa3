...
int hostapd_setup_interface(struct hostapd_iface *iface)
{
    ...
    /* --- TESE: Gerar Chaves Mestras --- */
    static int tese_keys_initialized = 0;
    if (!tese_keys_initialized) {
        u8 k_master[32];
        
        wpa_printf(MSG_INFO, "\033[1;35m[TESE] A gerar chaves mestras...\033[0m");
        
        if (os_get_random(k_master, 32) < 0) {
            wpa_printf(MSG_ERROR, "[TESE] Falha fatal ao gerar entropia!");
            return -1;
        }

        // 2. Derivar a Chave de Cifragem (Kap)
        // Usamos uma label diferente para cada derivação
        hmac_sha256(k_master, 32, (u8 *) "TESE-TICKET-ENC", 15, global_stek_enc);
        
        // 3. Derivar a Chave de Integridade (Kap-int)
        hmac_sha256(k_master, 32, (u8 *) "TESE-TICKET-MAC", 15, global_stek_mac);

        os_memset(k_master, 0, 32);

        tese_keys_initialized = 1;
        
        wpa_printf(MSG_DEBUG, "[TESE] Kap (ID): %02x%02x%02x%02x", 
                   global_stek_enc[0], global_stek_enc[1], global_stek_enc[2], global_stek_enc[3]);
    }
