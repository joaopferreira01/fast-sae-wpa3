...
u8 tese_salt_global[16];
int tese_salt_status = 0;
extern u16 cost;



/* Constrói o IE com o Salt e o custo 
 * Formato: [ID=221] [Len] [OUI=00:11:22] [Tipo=0x99] [Salt(16)] [Iterações(2)]
 */
u8 * hostapd_eid_sae_mitigation(struct hostapd_data *hapd, u8 *eid)
{
    if (tese_salt_status == 0) {
        os_get_random(tese_salt_global, 16);
        tese_salt_status = 1;
        wpa_hexdump(MSG_INFO, "TESE: Salt gerado:", tese_salt_global, 16);
    }

    cost = 150; 

    *eid++ = WLAN_EID_VENDOR_SPECIFIC; // ID 221
    *eid++ = 4 + 16 + 2;               // Length: 4 (OUI+Type) + 16 (Salt) + 2 (Cost) = 22
    ...
    *eid++ = 0x99; // OUI Type
    ...
    os_memcpy(eid, tese_salt_global, 16);
    eid += 16;

    // Usar BE16 (Big Endian) 
    WPA_PUT_BE16(eid, cost);
    eid += 2;
...
#define MAX_PROBERESP_LEN 768
	...
	*pos++ = hapd->conf->ssid.ssid_len;
	...
	pos = hostapd_eid_sae_mitigation(hapd, pos);
...
#define BEACON_TAIL_BUF_SIZE 512
	...
	*pos++ = WLAN_EID_SSID;
	...
	// Adicionei esta linha
	tailpos = hostapd_eid_sae_mitigation(hapd, tailpos);
